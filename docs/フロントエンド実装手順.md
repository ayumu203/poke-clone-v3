...
├── components/
│   ├── Battle/
│   │   ├── BattleField.tsx          # バトル画面全体
│   │   ├── PokemonDisplay.tsx       # ポケモン表示
│   │   ├── HPBar.tsx                # HPバー
│   │   ├── MoveSelector.tsx         # 技選択UI
│   │   ├── PokemonSelector.tsx      # ポケモン選択UI
│   │   ├── BattleLog.tsx            # バトルログ
│   │   └── TypeEffectivenessIcon.tsx # タイプ相性アイコン
│   └── Auth/
│       └── Login.tsx                 # ログイン画面
├── hooks/
│   ├── useBattle.ts                 # バトル状態管理フック
│   ├── useSignalR.ts                # SignalR接続フック
│   └── useAuth.ts                   # 認証フック
├── services/
│   ├── api.ts                       # HTTPクライアント
│   └── signalr.ts                   # SignalR接続管理
├── stores/
│   ├── battleStore.ts               # バトル状態ストア
│   └── authStore.ts                 # 認証状態ストア
├── types/
│   ├── battle.ts                    # バトル関連型定義
│   ├── pokemon.ts                   # ポケモン関連型定義
│   └── action.ts                    # アクション型定義
└── utils/
    ├── typeEffectiveness.ts         # タイプ相性計算
    └── damageCalculator.ts          # ダメージ計算（参考実装）
```
